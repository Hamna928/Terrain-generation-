...
def generate_terrain(terrain_type):
    if terrain_type == "Mountains":
        ...
        octaves, scale = 5, 0.45
        def postprocess(t):
            t = (t - t.min()) / (t.max() - t.min())
            return t * 0.001 + 0.4995
...
def generate_biome_map(terrain):
    ...
    for i in range(size):
        for j in range(size):
            h = terrain[i][j]
            if h < 0.3:
                biome_map[i][j] = [0.0, 0.4, 0.8]
            elif h < 0.4:
                biome_map[i][j] = [0.9, 0.8, 0.5]
            elif h < 0.6:
                biome_map[i][j] = [0.1, 0.8, 0.1]
            elif h < 0.75:
                biome_map[i][j] = [0.1, 0.5, 0.1]
            elif h < 0.9:
                biome_map[i][j] = [0.5, 0.5, 0.5]
            else:
                biome_map[i][j] = [1.0, 1.0, 1.0]
    return biome_map

def generate_maps_parallel(terrain):
    results = {}

    def compute_biome():
        results["biome"] = generate_biome_map(terrain)

    def compute_height():
        results["height"] = terrain

    def compute_temp():
        results["temperature"] = 1.0 - terrain

    def compute_moisture():
        results["moisture"] = (1 - terrain) ** 2

    def compute_topo():
        results["topo"] = terrain

    with ThreadPoolExecutor() as executor:
        executor.submit(compute_biome)
        executor.submit(compute_height)
        executor.submit(compute_temp)
        executor.submit(compute_moisture)
        executor.submit(compute_topo)
...
def show_3d_terrain(terrain, terrain_type):
    ...
    rgb = ls.shade(terrain, cmap=plt.get_cmap('gist_earth'), vert_exag=1, blend_mode='soft')
...
def on_biome():
    if current_maps.get("biome") is not None:
        plt.figure(figsize=(6, 6))
        plt.imshow(current_maps["biome"], origin='lower')
        plt.title("Biome Map")
        plt.axis('off')
        plt.show()

def on_height():
    if current_maps.get("height") is not None:
        plt.figure(figsize=(6, 6))
        plt.imshow(current_maps["height"], cmap='terrain', origin='lower')
...
def on_temp():
    if current_maps.get("temperature") is not None:
        plt.figure(figsize=(6, 6))
        plt.imshow(current_maps["temperature"], cmap='coolwarm', origin='lower')
...
def on_moisture():
    if current_maps.get("moisture") is not None:
        plt.figure(figsize=(6, 6))
        plt.imshow(current_maps["moisture"], cmap='Blues', origin='lower')
...
def on_topo():
    if current_maps.get("topo") is not None:
        plt.figure(figsize=(6, 6))
        plt.contourf(current_maps["topo"], cmap='Greens')
        plt.title("Topographic Map")
        plt.axis('off')
        plt.show()

def on_generate(terrain_type):
    global current_terrain, threads_used, current_maps
    current_terrain, threads_used = generate_terrain(terrain_type)
    current_maps = generate_maps_parallel(current_terrain)
...
plain_btn = ttk.Button(left_frame, text="Generate Plateau", width=btn_width, command=lambda: on_generate("Plateau"))
